...
public class FahrenheitToCelsiusFilter extends FilterFramework {
			...
			*************************************************************/
			...
			try
			{
				...
				if (frameReader.hasFrameAvailable()) {
					...
					convertFahrenheitToCelsius(frame);
	...
	private void convertFahrenheitToCelsius(Frame argFrame) {
		for (Measurement m : argFrame.getMeasurements()) {
			if (m.getMeasurementId() == MeasurementId.TEMPERATURE) {
				// convert fahrenheit to celsius
				m.setValue(((m.getValue()- 32) * 5) /9);
			}
		}
	}
